...
const GeneralSettingsModal: React.FC<GeneralSettingsModalProps> = ({
  ...
  const [refreshStoreOnStartup, setRefreshStoreOnStartup] = useState(() => {
    return localStorage.getItem('rh_refresh_store_startup') !== 'false';
  });

  const handleToggleRefreshStore = () => {
    const nextVal = !refreshStoreOnStartup;
    setRefreshStoreOnStartup(nextVal);
    localStorage.setItem('rh_refresh_store_startup', nextVal.toString());
  };
  ...
  return (
    ...
    >
      <div
        className="w-[640px] max-w-[95vw] max-h-[85vh] flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-[#1a1d24]"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto space-y-4 p-4 custom-scrollbar">
          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800/50 dark:bg-slate-900/30">
            <div className="mb-3">
              <h4 className="text-sm font-bold text-slate-800 dark:text-white">启动页设置</h4>
              ...
              {STARTUP_VIEW_OPTIONS.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-lg border py-2 px-3 text-center transition-all ${
                      isActive
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800/50 dark:bg-slate-900/30">
            <div className="mb-3">
              <h4 className="text-sm font-bold text-slate-800 dark:text-white">首页设置</h4>
              <p className="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">
                选择启动进入首页时默认展示的标签页，后续也可以随时回来修改。
              </p>
            </div>
              ...
              {HOME_TAB_OPTIONS.map((option) => {
                ...
                return (
                  <button
                    key={option.value}
                    type="button"
                    onClick={() => onUpdateHomeDefaultTab(option.value)}
                    className={`rounded-lg border py-2 px-3 text-center transition-all ${
                      isActive
                        ? 'border-brand-500 bg-brand-500 text-white shadow-md shadow-brand-500/20'
                        : 'border-slate-200 bg-white text-slate-700 hover:border-brand-300 hover:shadow-sm dark:border-slate-700 dark:bg-[#161920] dark:text-slate-200 dark:hover:border-brand-700'
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800/50 dark:bg-slate-900/30">
            <div className="flex items-center justify-between gap-3">
              <div className="flex-1">
                <h4 className="text-sm font-bold text-slate-800 dark:text-white">启动时刷新商城</h4>
                <p className="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">
                  开启后获取最新商城列表，关闭后使用缓存提升速度。
                </p>
              </div>
              <button
                type="button"
                onClick={handleToggleRefreshStore}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 dark:focus:ring-offset-[#1a1d24] ${
                  refreshStoreOnStartup ? 'bg-brand-500' : 'bg-slate-300 dark:bg-slate-600'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    refreshStoreOnStartup ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800/50 dark:bg-slate-900/30">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="flex-1">
                <h4 className="text-sm font-bold text-slate-800 dark:text-white">自动保存</h4>
                <p className="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">
                  任务完成后，自动将结果保存到指定目录。
                </p>
              </div>
