...
const FavoritesPanel: React.FC<FavoritesPanelProps> = ({ favorites, onSelectApp, apiKeys, onUpdateFavorites }) => {
    ...
    return (
        <div className="flex-1 flex flex-col bg-white dark:bg-[#161920] overflow-hidden relative">
            {toast && <Toast message={toast.message} type={toast.type} onClose={() => setToast(null)} />}

            <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-800/50 shrink-0">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors px-2 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800"
                        title="导入收藏"
                    >
                    ...
                    <button
                        ...
                        disabled={isSyncing}
                        className="flex items-center gap-1.5 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors px-2 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50"
                    >
                        {isSyncing ? (
                            <>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                        {favorites.map((fav) => {
                            ...
                            return (
                                <div
                                    key={fav.webappId}
                                    className="group relative flex flex-col bg-white dark:bg-[#1a1d24] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:border-brand-300 dark:hover:border-brand-700 transition-all duration-300 overflow-hidden cursor-pointer"
